...
def open_table_editor(app):
        ...
        def save_label(e):
            ...
            if not new_val:
                ...
                if is_row and new_val not in states:
                    # Обновляем имя состояния в списке
                    states[states.index(old_val)] = new_val
                    
                    # Обновляем ключи в словаре ячеек
                    for sym in symbols:
                        if (old_val, sym) in cell_fields:
                            cell_fields[(new_val, sym)] = cell_fields.pop((old_val, sym))
                            
                    # --- НОВАЯ ЛОГИКА: Обновляем значения ВНУТРИ всех ячеек ---
                    for key, tf in cell_fields.items():
                        if tf.value:
                            # Разбиваем по запятым и очищаем от пробелов
                            targets = [t.strip() for t in tf.value.split(',')]
                            # Заменяем точные совпадения старого имени на новое
                            updated_targets = [new_val if t == old_val else t for t in targets]
                            # Собираем обратно в строку
                            tf.value = ', '.join([t for t in updated_targets if t])
                    # -----------------------------------------------------------
    ...
    def delete_row(e):
        ...
        dd = ft.Dropdown(options=[ft.dropdown.Option(s) for s in states], width=200, autofocus=True)
        ...
        dlg = ft.AlertDialog(
            title=ft.Text("Удалить состояние"),
            content=dd,
            actions=[
                ft.ElevatedButton("Отмена", on_click=lambda e: (app.page.close(dlg), app.page.open(table_sheet), app.page.update())),
                ft.ElevatedButton("Удалить", on_click=confirm_delete, color=ft.Colors.RED)
            ]
        )
        app.page.close(table_sheet)
        app.page.open(dlg)

    def delete_column(e):
        if not symbols: return
        
        dd = ft.Dropdown(options=[ft.dropdown.Option(s) for s in symbols], width=200, autofocus=True)
        
        def confirm_delete(e):
            val = dd.value
            if val and val in symbols:
                symbols.remove(val)
                for state in states:
                    cell_fields.pop((state, val), None)
            app.page.close(dlg)
            build_table_ui()
            app.page.open(table_sheet)
            app.page.update()
    ...
    table_sheet = ft.BottomSheet(
        content=ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Row([
                    ft.ElevatedButton("Строка +", on_click=add_row, icon=ft.Icons.ADD),
                    ft.ElevatedButton("Строка -", on_click=delete_row, icon=ft.Icons.REMOVE, color=ft.Colors.RED),
                    ft.ElevatedButton("Столбец +", on_click=add_column, icon=ft.Icons.ADD),
                    ft.ElevatedButton("Столбец -", on_click=delete_column, icon=ft.Icons.REMOVE, color=ft.Colors.RED),

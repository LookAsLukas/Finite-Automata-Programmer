...
class Application:
    ...
    def show_info_dialog(self):
        """Показывает диалог со справкой по использованию приложения (без смайликов)."""
        help_lines = [
            ("FAP – Визуальный конструктор конечных автоматов", "bold", 18),
            ("", "", 0),
            ("Основные возможности", "bold", 17),
            ("", "", 0),
            ("Создание и редактирование автомата", "bold", 16),
            ("  • Режим добавления состояний – клик на пустом месте холста.", "", 15),
            ("  • Режим добавления переходов – выберите начальное состояние, затем конечное.", "", 15),
            ("  • Удаление – выделите узел или переход и нажмите «Удалить».", "", 15),
            ("  • Переименование – двойной клик по узлу.", "", 15),
            ("  • Редактирование перехода – двойной клик по дуге (можно ввести несколько символов, например `ab`).", "", 15),
            ("  • Начальное/конечное состояние – выделите узел и нажмите соответствующие кнопки.", "", 15),
            ("", "", 0),
            ("Алфавит", "bold", 16),
            ("  • Добавляйте или удаляйте символы. Они будут использоваться для переходов.", "", 15),
            ("", "", 0),
            ("Регулярные выражения", "bold", 16),
            ("  • Построить автомат из регулярного выражения (синтаксис automata-lib).", "", 15),
            ("  • Преобразовать автомат в регулярное выражение (метод исключения состояний).", "", 15),
            ("", "", 0),
            ("Табличный редактор", "bold", 16),
            ("  • Нажмите «Редактор таблицы» – удобное заполнение переходов в виде матрицы.", "", 15),
            ("  • Можно добавлять/удалять строки (состояния) и столбцы (символы).", "", 15),
            ("  • Двойной клик по заголовку строки/столбца – переименование.", "", 15),
            ("", "", 0),
            ("Масштаб и навигация", "bold", 16),
            ("  • Кнопки «+»/«–», слайдер для изменения масштаба холста.", "", 15),
            ("  • Перетаскивание узлов мышью (зажмите и ведите).", "", 15),
            ("", "", 0),
            ("Проверка слов", "bold", 16),
            ("  • Введите слово в поле и нажмите «Обработать слово».", "", 15),
            ("  • Результат (принимается/отвергается) отобразится в строке статуса.", "", 15),
            ("", "", 0),
            ("Отладка (пошаговое выполнение)", "bold", 16),
            ("  • Нажмите «Отладка», введите слово – откроется панель управления.", "", 15),
            ("  • «Шаг вперёд» – обработать один символ, «Шаг назад» – откат, «Продолжить» – автоматический прогон.", "", 15),
            ("  • Текущие состояния подсвечиваются жёлтым цветом.", "", 15),
            ("", "", 0),
            ("Оптимизация", "bold", 16),
            ("  • «Оптимизировать (Min DFA)» – преобразует НКА в ДКА и минимизирует его.", "", 15),
            ("", "", 0),
            ("Файловые операции (меню «Файл»)", "bold", 16),
            ("  • Сохранить автомат в JSON (включая регулярное выражение).", "", 15),
            ("  • Открыть ранее сохранённый автомат.", "", 15),
            ("", "", 0),
            ("История действий", "bold", 16),
            ("  • Кнопки «Undo» / «Redo» откатывают или возвращают изменения.", "", 15),
            ("", "", 0),
            ("Совет: для большинства операций сначала выберите объект (узел или переход) – он будет подсвечен синим.", "italic", 14),
        ]

        children = []
        for text, weight, size in help_lines:
            if not text:
                children.append(ft.Container(height=5))  # небольшой отступ
                continue
            ...
            if weight == "bold":
                ...
                style.italic = True
            children.append(ft.Text(text, style=style))

        content = ft.Container(
            content=ft.Column(children, scroll=ft.ScrollMode.ADAPTIVE, spacing=6),
            width=700,
            height=450,
            padding=15,
        )
        ...
        dialog = ft.AlertDialog(
            title=ft.Text("FAP - Finite Automata Programmer", size=20, weight=ft.FontWeight.BOLD),
            content=content,
            actions=[ft.ElevatedButton("Закрыть", on_click=lambda e: self.page.close(dialog))],
